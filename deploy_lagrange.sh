...
# 与菜单一一对应的 URL 常量
lorana_cloudflare="https://cf-sign.xuetao.host/42941"
lorana_backbone="https://backbone.seal-sign.xuetao.host/sign/42941"
lorana_edge="https://edge.seal-sign.xuetao.host/sign/42941"
lorana_turbo="https://turbo.seal-sign.xuetao.host/sign/42941"
lorana_proxy_backup="http://39.108.115.52:58080/api/sign/39038"

case "${sign_choice:-}" in
  1) sign_server_url="$lorana_cloudflare" ;;
  2) sign_server_url="$lorana_backbone" ;;
  3) sign_server_url="$lorana_edge" ;;
  4) sign_server_url="$lorana_turbo" ;;
  5) sign_server_url="$lorana_proxy_backup" ;;
  *) echo -e "${YELLOW}未选择有效编号，默认使用：${BLUE}雪桃家自签 - Cloudflare${NC}"; sign_server_url="$lorana_cloudflare" ;;
esac
